...
This can be overridden with the "-basedir" command line parm to allow code debugging in a different directory.  The base directory is only used during filesystem initialization.
...
This can be overridden with the "-game" command line parameter.  The game directory can never be changed while quake is executing.
...
void FS_Read(void* buffer, int len, FILE* f)
{
	...
	while (remaining)
	{
		...
		if (block > MAX_READ)
			block = MAX_READ;
		read = static_cast<int>(fread(buf, 1, block, f));
		if (read == 0)
		{
...
void FS_Link_f(void)
{
	...
	l->fromlength = static_cast<int>(strlen(l->from));
	l->to = CopyString(Cmd_Argv(2));
}
